...
export default class UpdateCallbackHandler extends FireblocksBaseCommand {
  static summary = 'Update API key callback handler'

  static description = 'Update the callback handler of an API key\nEndpoint Permission: Admin and Non-Signing Admin.\n\nOperation ID: updateCallbackHandler\nDocs: https://docs.fireblocks.com/api/swagger-ui/#/Cosigners/updateCallbackHandler'
  ...
  async run(): Promise<unknown> {
    const {flags} = await this.parse(UpdateCallbackHandler)
    ...
    await this.confirmOrAbort('PATCH', '/v1/cosigners/{cosignerId}/api_keys/{apiKeyId}')
